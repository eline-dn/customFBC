    ...
    if not trajectory_exists:
        ...
        if trajectory.aux["log"]["terminate"] == "":
            ...
            if advanced_settings["enable_mpnn"]:
                ...
                restricted_AAs = set(aa.strip().upper() for aa in (advanced_settings["omit_AAs"] if advanced_settings["omit_AAs"] else "").split(',') if aa.strip()) if advanced_settings["force_reject_AA"] else set()
                ...
                if mpnn_sequences:
                    ...
                    # check if monomer filters are set
                    use_monomer_filter = any(v.get("threshold") is not None for k, v in filters.items() if any(metric in k for metric in ["Binder_pLDDT", "Binder_pTM", "Binder_pAE", "Binder_RMSD"]))

                    if use_monomer_filter:
                    ...
                    for mpnn_sequence in mpnn_sequences:
                        ...
                        if use_monomer_filter:
                            ...
                            binder_averages = calculate_averages(binder_statistics)
                                ...
                                # Initialize binder_averages with same keys and None values
                                binder_averages = {
                                    'pLDDT': None,
                                    'pTM': None,
                                    'pAE': None,
                        ...
                        ### TODO: make this work with column names not positions
