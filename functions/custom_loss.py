...
def iptm_contrib_variant_A(logits, breaks, asym_id, residue_weights=None, subset_S=[0], use_jnp=True):
    ...
    Inputs
    -------
    logits            : [N, N, B] PredictedAlignedErrorHead logits
    breaks            : [B-1] AF bin breaks
    residue_weights   : [N] float mask in [0,1]
    asym_id           : [N] chain-asym IDs (ints)
    subset_S          : e.g. [12, 13, 14, 57, 58, 59] 

    Returns
    -------
    contrib_abs_S     : scalar, contribution of S to global iPTM
    """
    ...
    if use_jnp:
      ...
      _np, _softmax = np, scipy.special.softmax
    # --- 1) Compute bin centers and TM weights exactly as AF does ---
    ...
    # Compute d_0(num_res) as defined by TM-score, eqn. (5) in Yang & Skolnick
    # "Scoring function for automated assessment of protein structure template
    # quality", 2004: http://zhanglab.ccmb.med.umich.edu/pap
    d0 = 1.24 * (clipped_num_res - 15) ** (1./3) - 1.8
    # Convert logits to probs.
    probs = _softmax(logits, axis=-1)
    ...
    return (per_alignment * residue_weights).max()"""
    # what is S/ subset mask:
    S = np.array(subset_S, dtype=int) # subset_S= [12, 13, 14, 57, 58, 59] e.g. 
    subset_mask = np.zeros((num_res, num_res), dtype=float)
    subset_mask[S, :] = 1.0
    subset_pair_mask = subset_mask * pair_mask

    # --- NEW: contribution numerator restricted to S ---
    pair_contrib = predicted_tm_term * pair_residue_weights * subset_mask
    
    # global denominator (same as in iPTM)
    N_interface = pair_residue_weights.sum()
    
    return pair_contrib.sum() / (N_interface + 1e-8)
"""
...
#----- same as af until now-------
    ...
    # --- 4) Interface mask ---
    inter_mask = (asym_id[:,None] != asym_id[None,:])  # bool [N,N]

    # --- 5) Outer-product residue weights ---
    w_i_w_j = residue_weights[:,None] * residue_weights[None,:]

    # --- 6) Global normalizer N_interface ---
    N_interface = jnp.sum(E_ij * w_i_w_j * inter_mask)

    # --- 7) Subset mask S × partner(S) ---
    # We assume residues in S belong to one chain,
    # so partner residues = all residues of the other chain.
    S_mask = subset_S[:,None]          # i ∈ S
    partner_mask = inter_mask & S_mask  # (i in S) and (i,j is interface)

    # partner_mask selects (i ∈ S, j ∈ interface partners)
    # weights for those pairs:
    weighted_pairs = E_ij * w_i_w_j * partner_mask

    numerator = jnp.sum(weighted_pairs)

    # --- 8) Variant A ---
    return numerator / (N_interface + 1e-8)
    """
